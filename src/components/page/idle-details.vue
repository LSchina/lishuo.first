    ...
    export default {
        ...
        created(){
            ...
            this.$api.getIdleItem({
                ...
                console.log(res);
                if(res.data){
                    let list=res.data.data.idleDetails.split(/\r?\n/);
                    let str='';
                    for(let i=0;i<list.length;i++){
                        str+='<p>'+list[i]+'</p>';
                    }
                    res.data.data.idleDetails=str;
                    res.data.data.pictureList=JSON.parse(res.data.data.pictureList);
                    this.idleItemInfo=res.data.data;
        ...
        methods: {
            ...
            buyButton(idleItemInfo){
                this.$api.addOrder({
                    ...
                    console.log(res);
                    if(res.data.status_code===1){
                        this.$router.push({path: '/order', query: {id: res.data.data.id}});
                    }else {
                        this.$message.error(res.data.msg)
                    }
                }).catch(e=>{
                ...
                if(content){
                    ...
                    this.$api.sendMessage({
                        ...
                        if(res.data.status_code===1){
                            ...
                            this.$message.error("留言失败！"+res.data.msg);
                        }
                    }).catch(()=>{
