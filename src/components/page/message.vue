        ...
        this.$api.getAllMyMessage().then(res => {
          ...
          if (res.data.status_code === 1) {
            for (let i = 0; i < res.data.data.length; i++) {
              let imgList = JSON.parse(res.data.data[i].idle.pictureList);
              res.data.data[i].idle.imgUrl = imgList ? imgList[0] : '';
              let contentList = res.data.data[i].content.split('<br>');
              let contenHtml = contentList[0];
              for (let i = 1; i < contentList.length; i++) {
                contenHtml += '  ' + contentList[i];
              }
              res.data.data[i].content = contenHtml;
            }
