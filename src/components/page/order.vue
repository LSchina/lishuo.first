        ...
        created() {
            ...
            this.$api.getOrder({
                ...
                if (res.data.status_code === 1) {
                    if (res.data.data.idleItem) {
                        let imgList = JSON.parse(res.data.data.idleItem.pictureList);
                        if (imgList.length > 0) {
                            res.data.data.idleItem.imgUrl = imgList[0];
                        } else {
                            res.data.data.idleItem.imgUrl = '';
                        }
                    } else {
                        res.data.data.idleItem = {
                            idleName: '',
                            imgUrl: ''
                        }
                    }
            ...
            selectAddress(i,item){
                ...
                if(this.addressInfo.update){
                    ...
                    this.$api.addOrderAddress({
                        ...
                        if(res.data.status_code===1){
                            this.addressInfo.update=true;
                            this.addressInfo.id=res.data.data.id;
                        }else {
                        ...
                        this.$confirm('模拟支付宝支付，是否确认支付', '支付订单', {
                            ...
                            this.$api.updateOrder({
                                ...
                                if (res.data.status_code === 1) {
                                    ...
                                    this.orderInfo.paymentTime = res.data.data.paymentTime;
                                }
                            })
                        }).catch(() => {
                        });
