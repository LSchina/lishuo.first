...
import com.second.hand.trading.server.model.OrderCount;
...
public class AdminController {
    ...
    public ResultVo messageDelete(HttpSession session,
                                  @RequestParam("id") @NotNull @NotEmpty Long id){
        if(session.getAttribute("admin")==null){
            return ResultVo.fail(ErrorMsg.COOKIE_ERROR);
        }
        if(messageService.deleteAdminMessage(id)){
            return ResultVo.success();
        }
        return ResultVo.fail(ErrorMsg.SYSTEM_ERROR);
    }
    ...
    @GetMapping("/data")
    public ResultVo data(HttpSession session){
        if(session.getAttribute("admin")==null){
            return ResultVo.fail(ErrorMsg.COOKIE_ERROR);
        }
        List<Integer> data = new ArrayList<>();
        data.add(userDao.countBanUser() + userDao.countNormalUser());
        data.add(idleItemDao.getAll().size());
        data.add(orderDao.countAllOrder());
        data.add(messageDao.countMessage());
        return ResultVo.success(data);
    }


    @GetMapping("/order")
    public ResultVo orderData(HttpSession session){
        if(session.getAttribute("admin")==null){
            return ResultVo.fail(ErrorMsg.COOKIE_ERROR);
        }
        List<OrderCount> orderCounts = orderDao.countOrderByStatus();
        List<Integer> orderData = orderCounts.stream().map(OrderCount::getCount).collect(Collectors.toList());
        if (CollUtil.isEmpty(orderData)){
            return null;
        }
        return ResultVo.success(orderData);
    }
}
